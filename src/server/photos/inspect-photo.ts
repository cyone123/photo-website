...
import { mimeTypeForExtension, storageExtension } from "./source-format";
...
export interface InspectedPhotoBuffer {
  buffer: Buffer;
  originalFilename: string;
  sourceLabel: string;
...
export async function inspectPhotoBuffer(
  buffer: Buffer,
  sourceInfo: PhotoSourceInfo,
): Promise<InspectedPhotoBuffer> {
  const originalFilename = sourceInfo.originalFilename.trim();
  const sourceLabel = sourceInfo.sourceLabel?.trim() || originalFilename || "image buffer";
  ...
  const extension = storageExtension(originalFilename, format);

  return {
    buffer,
    originalFilename,
    sourceLabel,
    declaredMimeType: sourceInfo.mimeType?.trim() || null,
